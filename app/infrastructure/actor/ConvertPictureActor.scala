...
class ConvertPictureActor @Inject()(
  ...
  val storeDirPath: Path = FileSystems.getDefault.getPath(convertStoreDirPath)
  if (!Files.exists(storeDirPath)) Files.createDirectories(storeDirPath)

  val imageMagickPath: String = configuration.get[String]("imagemagick.path")
  val imageMagickFontPath: String = configuration.get[String]("imagemagick.fontpath")
  ...
  private[this] def convert(pictureProperty: PictureProperty,
    ...
    val convertedPictureProperty: PictureProperty = Try {
      ...
      case Failure(t) => {
        Logger("access").error("Fail to convert.", t)
    ...
    picturePropertyRepository.update(
      ...
      case Success(_) => Logger("access").info(s"Converted and updated. convertedPictureProperty: ${convertedPictureProperty}")
      case Failure(t) => Logger("access").error("Fail to update.", t)
    }
  }
